...
import Navbar from '../MatchNavBar/index'
...
class MatchGame extends Component {
  ...
  startNewGame = () => {
    this.setState({
      ...
      timeLeft: 60,
      activeTabId: 'FRUIT',
      thumbnailImage: imagesList[Math.floor(Math.random() * imagesList.length)],
      isTimeStart: true,
      isGameOver: false,
    })
  }
        ...
        <ul className="image-list-container">
          {filteredImageList.map(eachImage => (
            <ImageItem
              eachImage={eachImage}
              key={eachImage.id}
              checkThumnailWithImage={this.checkThumnailWithImage}
            />
          ))}
        </ul>
      </div>
    )
    const displayBottomCard = isGameOver ? (
      <GameOver score={score} startNewGame={this.startNewGame} />
    ) : (
